...
def apply_rotary_pos_emb_with_cos_sin(
    ...
    """Apply RoPE when queries and keys use different position sets."""
...
def _normalize_position_ids(
    ...
    if position_ids.shape[0] != batch_size:
        raise ValueError(
            f"position_ids batch dimension {position_ids.shape[0]} does not match input batch size {batch_size}."
        )
    return position_ids


def _position_ids_from_replace_position(
    ...
    counts = replace_position.sum(dim=1)
    if not torch.all(counts == expected_len):
        return None

    gathered = [
        replace_position[batch_idx].nonzero(as_tuple=True)[0]
    ...
    def forward(
        ...
        if past_key_value is not None:
            if dual_cache:
                ...
                if replace_position is None:
                    raise ValueError("replace_position must be provided when dual_cache=True and past_key_value is set.")

                input_len = key_states.shape[1]
                cache_len = past_key.shape[1]
                for batch_idx in range(bsz):
                    batch_replace_indices = replace_position[batch_idx].nonzero(as_tuple=True)[0]
                    num_replace = batch_replace_indices.numel()
                    if num_replace == 0:
                        continue

                    if input_len == cache_len:
                        past_key[batch_idx, batch_replace_indices] = key_states[batch_idx, batch_replace_indices]
                        past_value[batch_idx, batch_replace_indices] = value_states[batch_idx, batch_replace_indices]
                    elif input_len == num_replace:
                        past_key[batch_idx, batch_replace_indices] = key_states[batch_idx, :num_replace]
                        past_value[batch_idx, batch_replace_indices] = value_states[batch_idx, :num_replace]
                    else:
                        raise ValueError(
                            "dual_cache update expects input_len to equal either cache_len or "
                            f"the number of replacement positions, got input_len={input_len}, "
                            f"cache_len={cache_len}, num_replace={num_replace}."
                        )
        ...
        if dual_cache:
            position_ids = _normalize_position_ids(position_ids, bsz, hidden_states.device)
            cache_position_ids = _normalize_position_ids(cache_position_ids, bsz, hidden_states.device)
            if position_ids is None:
                position_ids = _position_ids_from_replace_position(replace_position, q_len)
            if position_ids is None:
                if past_key_value is not None:
                    start = key_states.shape[-2] - q_len
            ...
            k_cos, k_sin = self.rotary_emb(key_states, key_position_ids)
            query_states, key_states = apply_rotary_pos_emb_with_cos_sin(
                query_states,
                key_states,
                q_cos,
                q_sin,
...
class DreamBaseModel(DreamPreTrainedModel):
    ...
    def forward(
        ...
        position_ids = _normalize_position_ids(position_ids, inputs_embeds.shape[0], inputs_embeds.device)
        if position_ids is None and dual_cache:
            position_ids = _position_ids_from_replace_position(replace_position, inputs_embeds.shape[1])
        if position_ids is None and not dual_cache:
            position_ids = torch.arange(
        ...
        position_embeddings = None if dual_cache else self.rotary_emb(hidden_states, position_ids)
