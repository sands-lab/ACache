...
for anchor_ratio in "${anchor_ratios[@]}"; do
  model_args="pretrained=Dream-org/Dream-v0-Instruct-7B,gen_length=256,steps=256,block_length=32,threshold=0.9,show_speed=True,affix_type=${affix_type},anchor_ratio=${anchor_ratio},selection_mode=top,cacheblend_score_metric=${score_metric},cacheblend_include_values=${include_values}"
  ...
  echo "Running CacheBlend-style selector with seed=${seed}, dataset=${dataset}, num_fewshot=${num_fewshot}, affix_type=${affix_type}, anchor_ratio=${anchor_ratio}, score_metric=${score_metric}, include_values=${include_values}, drop_non_anchor=${drop_non_anchor}, confirm_run_unsafe_code=${confirm_run_unsafe_code}"
