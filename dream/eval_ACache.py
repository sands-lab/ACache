"""
Evaluation script for ACache with optional prefix/suffix/infix few-shot affix.
"""
...
from generate_ACache import generate_with_anchor_attention
...
@register_model("dream_acache")
class DreamAnchorEvalHarness(ACacheEvalHarnessMixin, LM):
    def __init__(
        self,
        pretrained="",
        mask_id=None,
        ...
        **kwargs,
        ...
        removed_knobs = {
            "model_path": "`model_path` has been removed from dream_acache; use `pretrained=...`.",
            "use_cache": "`use_cache` has been removed from dream_acache; cached block decoding is always enabled.",
            "dual_cache": "`dual_cache` has been removed from dream_acache; dual-cache decoding is always enabled.",
            "use_anchor": (
                "`use_anchor` has been removed from dream_acache; use `anchor_ratio=0` for the "
                "frozen-affix ablation or `affix_type=none` for the no-affix baseline."
            ),
            "print_fewshot_affix": "`print_fewshot_affix` has been removed from dream_acache.",
        }
        ...
        if self.seed is not None:
            set_seed(self.seed)
            print(f"Manual seed set in eval_ACache: {self.seed}")
    ...
    def _generate_with_affix_cache(
        ...
        return generate_with_anchor_attention(self.model, input_ids, **generation_kwargs)
