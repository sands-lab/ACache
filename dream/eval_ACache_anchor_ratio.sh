...
for anchor_ratio in "${anchor_ratios[@]}"; do
  ...
  accelerate launch eval_ACache.py \
    ...
    --trust_remote_code \
    --model dream_acache \
    --model_args "pretrained=${model},gen_length=256,steps=256,block_length=32,threshold=0.9,show_speed=True,affix_type=prefix,anchor_ratio=${anchor_ratio},selection_mode=top"
done
