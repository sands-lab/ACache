"""
CacheBlend-style Anchor selection for LLaDA ACache.

This sidecar module leaves ``generate_ACache.py`` unchanged.  It keeps ACache's
generation path intact and replaces only the one-shot Anchor selection score with
a CacheBlend-inspired high-KV-deviation oracle.
"""
...
def _token_deviation(
    ...
    if metric == "l2":
        return torch.linalg.vector_norm(diff, ord=2, dim=-1).sum(dim=1)
    if metric == "l1":
        return diff.abs().sum(dim=-1).sum(dim=1)
    if metric == "relative_l2":
        numerator = torch.linalg.vector_norm(diff, ord=2, dim=-1)
        denominator = torch.linalg.vector_norm(cached_tensor, ord=2, dim=-1).clamp_min(eps)
        return (numerator / denominator).sum(dim=1)
...
def compute_cacheblend_kv_deviation_scores(
    ...
    """
    Score affix tokens by KV deviation.

    LLaDA caches use shape ``(batch, heads, seq, head_dim)``.  Scores are summed
    over heads and layers, yielding ``(batch, affix_len)``.
    """
    del mask_id
    ...
    affix_len = affix_cache[0][0].shape[2]
    ...
    for layer_idx, ((full_k, full_v), (cached_k, cached_v)) in enumerate(zip(full_cache, affix_cache)):
        if full_k.dim() != 4 or full_v.dim() != 4 or cached_k.dim() != 4 or cached_v.dim() != 4:
            raise ValueError(
                "CacheBlend-style LLaDA scorer expects cache tensors with shape "
                f"(batch, heads, seq, head_dim); got layer {layer_idx} shapes "
...
def generate_with_cacheblend_anchor_attention(
    ...
    """
    Run LLaDA ACache with a one-shot CacheBlend-style HKVD Anchor selector.
    """
    ...
    def cacheblend_selector(model_arg, full_input_ids, affix_cache, mask_id, affix_start=0):
        local_affix_len = affix_cache[0][0].shape[2] if affix_cache else 0
