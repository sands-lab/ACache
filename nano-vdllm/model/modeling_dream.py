    ...
    def forward(self, x, position_ids):
        if position_ids.dim() == 1:
            ...
            if "dynamic" in self.rope_type:
                self._dynamic_frequency_update(position_ids.unsqueeze(0), device=x.device)
...
class DreamSdpaAttention(DreamAttention):
    ...
    def forward(
        ...
        if output_attentions:
            raise NotImplementedError("Dream paged attention does not support output_attentions.")

        is_ragged = hidden_states.dim() == 2
        query_states = self.q_proj(hidden_states)
        key_states = self.k_proj(hidden_states)
        value_states = self.v_proj(hidden_states)
        ...
        if is_ragged:
            ...
            if position_ids is None:
                position_ids = torch.arange(total_tokens, device=hidden_states.device, dtype=torch.long)
            elif position_ids.dim() != 1:
                position_ids = position_ids.reshape(-1)

            query_states = query_states.view(total_tokens, self.num_heads, self.head_dim)
...
class DreamModel(DreamPreTrainedModel):
    ...
    def forward(
        ...
        logits_keep_indices: Optional[torch.LongTensor] = None,
        **loss_kwargs,
        ...
        logits = None
        if compute_logits:
            if logits_keep_indices is not None:
                logits_keep_indices = logits_keep_indices.to(device=hidden_states.device, dtype=torch.long)
                hidden_states_for_logits = hidden_states.reshape(-1, hidden_states.shape[-1])[logits_keep_indices]
            elif num_logits_to_keep and hidden_states.dim() == 3:
                hidden_states_for_logits = hidden_states[:, -num_logits_to_keep:, :]
            else:
